...
double discrete(double min, double max, int levels, double val) {
	max = max - min;
	val = val - min;
	levels--;

	val = (int) ((val * levels / max) + 0.5);
	return (val * max / levels) + min;
}
	...
	for (i=0; i<num; i++) {
		for (j=0; j < num; j++) {
			if (*uPtrs2[j*num + i] == 1) {
                ...
                (y0)[j*num + i] = discrete(0, 1, 11, (y0)[j*num+i]);
				/* adaptation law - confidence --sigma*/
				if (*uPtrs1[0] > 0)
					(y1)[j*num + i] = - (*uPtrs6[0]) * sign((y2)[j*num + i] - *uPtrs3[j*num + i]) ;  
				else
					(y1)[j*num + i] = 0; 
                
                (y1)[j*num + i] = discrete(0, 1, 11, (y1)[j*num+i]);
            }
		}
	}
}
