...
for time = 1 : 100 : length(t)
    T = [T; t(time)];
    zetas = [zetas; zeros(1,n*n)];
    for i = 1 : n
        zetas(index,(i-1)*n + 1 : i*n) = zeta(i,:,time);
    end        
    index = index + 1;
end
...
for i = 1:n
    ...
    cstring=[];
    for j = 0:n-1
        plot(T, zetas(:,i+j*n), 'LineWidth', 1.25);
        hold on
        cstring{end+1} = sprintf('\\zeta_{%d%d}',j+1,i);
    end
    
    legend(cstring, 'FontSize', 8, 'Orientation', 'Horizontal', 'Location', 'South')
    grid on 
    xlabel('t(s)')
    ylabel('trust')
    title(sprintf('Trust towards agent %d', i));
    axis([tmin tmax 0 1])
end
